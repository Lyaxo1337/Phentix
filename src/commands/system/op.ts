...
const op: Command = {
  ...
  run: async (_client: Client, commandData: CommandData) => {
    ...
    commandData.channel.isText()
      ? await commandData.channel.bulkDelete(1)
      : console.error("This has to be a text channel you fucking idiot");
    commandData.channel.send("Gave Operator Permissions").then((msg) =>
      setTimeout(() => {
        msg.delete();
      }, 1000)
    );
